...
import { SellerProductEditDrawer } from "@/components/seller/SellerProductEditDrawer";
...
export function SellerCatalogDesk({
  ...
  function upsertProduct(product: SellerCatalogProduct) {
    setProducts((prev) => {
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        ...
        <StatCard
          ...
          value={formatCurrency(stats.totalMargin, "ZAR")}
          hint="Retail minus cost (per unit)"
        ...
        <StatCard
          ...
          hint={stats.low ? `${stats.low} low stock` : "Across all SKUs"}
      ...
      {activeTab === "products" ? (
        <SellerCatalogProductsTab
          ...
          onListingUpdated={(id, listingStatus) =>
            setProducts((prev) =>
              prev.map((p) => (p.id === id ? { ...p, listing_status: listingStatus } : p)),
            )
          }
          onArchived={(id) => setProducts((prev) => prev.filter((p) => p.id !== id))}
        />
      ) : null}
      {activeTab === "stock" ? (
