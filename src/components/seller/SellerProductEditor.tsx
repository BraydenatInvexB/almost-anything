...
export function SellerProductEditor({
  ...
  const [markupPercent, setMarkupPercent] = useState(
    product ? String(Number(product.markup_percent) || shipping.defaultMarkupPercent) : String(shipping.defaultMarkupPercent),
  );
  ...
  const [delivery, setDelivery] = useState<SellerDeliverySettings>(() =>
    product ? parseSellerDelivery(product.metadata) : { customerPaysDelivery: true, deliveryFeeZar: null },
  );
    ...
    <div className="space-y-4">
      <section className="rounded-xl border border-neutral-200 bg-white p-5 shadow-sm">
        ...
        <p className="mt-0.5 text-xs text-neutral-500">
          {mode === "edit"
            ? "Update photos, pricing, and details — then save as draft or list when ready."
